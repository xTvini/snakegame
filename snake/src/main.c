...
int main() {
    screenInit(0);
    keyboardInit();
    timerInit(100 * difficulty); // Velocidade padrão do jogo
    int escolha = painel_de_escolha_inicio();
    while (escolha != 4) { // Loop para manter o jogo em execução até que o jogador escolha sair
        if (escolha == 1) {
            screenInit(0);
            ...
            // Implementar a função de instruções de como jogar
        }
        screenInit(0);
